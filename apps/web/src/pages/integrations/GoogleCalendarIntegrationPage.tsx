...
export const GoogleCalendarIntegrationPage: React.FC = () => {
  return (
    <div style={{ paddingBottom: 'var(--space-80)' }}>
      <SEOHead path="/integrations/google-calendar" />
      ...
      <section className="section" style={{ paddingTop: 'var(--space-32)', paddingBottom: 'var(--space-16)' }}>
        <div className="container" style={{ maxWidth: '1000px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12.5px', color: 'var(--text-muted)' }}>
            ...
            <span>/</span>
            <span style={{ color: 'var(--cyan)', fontWeight: 500 }}>Google Calendar</span>
          </div>
        </div>
      </section>
        ...
        <div className="container" style={{ maxWidth: '1000px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: 'var(--space-20)' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '10px', backgroundColor: 'var(--surface-raised)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <ServiceLogo name="google-calendar" size={30} />
            </div>
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>· OAuth 2.0 PKCE · REST API v3</span>
              </div>
            </div>
          </div>

          <h1 className="heading-1" style={{ fontSize: 'clamp(32px, 5vw, 46px)', color: 'var(--text)', marginBottom: 'var(--space-16)', letterSpacing: '-0.02em' }}>
            Connect scheduling to the work around it.
          </h1>

          <p style={{ fontSize: '18px', color: 'var(--text-muted)', lineHeight: 1.6, maxWidth: '780px', margin: 0 }}>
            Syntaflow synchronizes Google Calendar with client milestones, delivery gate reviews, and presentation sessions, ensuring schedule commitments remain firmly tied to active project blueprints.
          </p>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', marginTop: 'var(--space-24)' }}>
            ...
            <Button variant="secondary" href="/privacy#sec-8">
              Review Calendar Scopes & Privacy
            </Button>
          </div>
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'var(--space-24)' }}>
            ...
            <Card variant="default" style={{ padding: 'var(--space-28)' }}>
              ...
              <ul style={{ paddingLeft: '18px', margin: 0, color: 'var(--text-muted)', fontSize: '14px', lineHeight: 1.7 }}>
                <li><strong>Milestone Scheduling:</strong> Automatically map deliverable deadlines and presentation reviews to calendar dates.</li>
                <li><strong>Availability Verification:</strong> Inspect free/busy scheduling windows to propose meeting times that prevent double-booking.</li>
                <li><strong>Client Project Links:</strong> Attach client scoping documents and meeting agendas directly to calendar invites.</li>
                <li><strong>Authorized Actions:</strong> Schedule presentation meetings with client attendees upon operator request.</li>
              </ul>
            </Card>
            ...
            <Card variant="default" style={{ padding: 'var(--space-28)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <div style={{ backgroundColor: 'var(--surface-sunken)', padding: '10px 12px', borderRadius: '6px', border: '1px solid var(--border)' }}>
                  <code style={{ fontSize: '12px', color: 'var(--cyan)' }}>calendar.readonly</code>
                  <div style={{ fontSize: '12.5px', color: 'var(--text-muted)', marginTop: '4px' }}>
                    Read calendars and events to display engagement timelines and check availability.
                  </div>
                </div>
                <div style={{ backgroundColor: 'var(--surface-sunken)', padding: '10px 12px', borderRadius: '6px', border: '1px solid var(--border)' }}>
                  <code style={{ fontSize: '12px', color: 'var(--cyan)' }}>calendar.events</code>
                  <div style={{ fontSize: '12.5px', color: 'var(--text-muted)', marginTop: '4px' }}>
                    Create and update client review sessions when authorized by the user.
                  </div>
                </div>
                <div style={{ backgroundColor: 'var(--surface-sunken)', padding: '10px 12px', borderRadius: '6px', border: '1px solid var(--border)' }}>
                  <code style={{ fontSize: '12px', color: 'var(--cyan)' }}>userinfo.email</code>
                  <div style={{ fontSize: '12.5px', color: 'var(--text-muted)', marginTop: '4px' }}>
                    Identify the active Google account for session verification.
                  </div>
                </div>
              </div>
            </Card>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-32)' }}>
            ...
            <div>
              <h2 style={{ fontSize: '24px', fontWeight: 600, color: 'var(--text)', marginBottom: 'var(--space-12)' }}>
                How It Connects & Data Security
              </h2>
              <p style={{ fontSize: '15px', color: 'var(--text-muted)', lineHeight: 1.7, margin: 0 }}>
                Syntaflow authenticates via Google OAuth 2.0 with PKCE in your system browser. Tokens are saved strictly in your local operating system keychain. Calendar API calls occur directly from your desktop to Google&rsquo;s API servers (<code>www.googleapis.com/calendar/v3</code>), with zero intermediate proxying or logging on Syntaflow cloud infrastructure.
              </p>
            </div>
            ...
            <div>
              ...
              <p style={{ fontSize: '15px', color: 'var(--text-muted)', lineHeight: 1.7, margin: 0 }}>
                Calendar event records are cached ephemerally in memory during active desktop sessions and are not uploaded to remote databases. You can disconnect Calendar at any time in application settings, which deletes all stored OAuth credentials from your device, or revoke access from your <a href="https://myaccount.google.com/permissions" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--cyan)' }}>Google Account Permissions Dashboard</a>.
              </p>
            </div>
        ...
        <div className="container" style={{ maxWidth: '1000px' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
            <div style={{ display: 'flex', gap: '16px', fontSize: '13.5px' }}>
              ...
              <Link href="/product/projects" style={{ color: 'var(--cyan)', textDecoration: 'none' }}>Projects & Milestones</Link>
            </div>
            <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
              Related: <Link href="/integrations/gmail" style={{ color: 'var(--text-muted)' }}>Gmail</Link> · <Link href="/integrations/google-drive" style={{ color: 'var(--text-muted)' }}>Google Drive</Link>
            </div>
          </div>
        </div>
      </section>
