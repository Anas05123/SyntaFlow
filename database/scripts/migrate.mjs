...
const migrationPath = path.join(repoRoot, "database", "migrations", "0001_foundation_metadata.sql");
const migration = {
...
if (databaseExisted) {
  ...
  backupPath = path.join(backupDirectory, `coredesk-before-migration-${timestamp}.sqlite`);
...
if (!alreadyApplied) {
  ...
  try {
    migration.sql
      .split(/-- (?:coredesk|atlas):statement-breakpoint/)
